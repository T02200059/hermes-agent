...
// Under Clear the lever IS the opacity; under Glass only `fade` reaches the
// native window (see windowOpacityFor).
function windowOpacity() {
  const lever =
    translucencyState.mode !== 'glass'
      ? translucencyState.intensity
      : glassActive(translucencyState)
        ? translucencyState.fade
        : 0
  return opacityForIntensity(lever, IS_WINDOWS)
}
