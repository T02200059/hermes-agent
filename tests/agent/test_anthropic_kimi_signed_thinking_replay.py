"""Third-party Anthropic endpoints preserve unsigned thinking, not signatures."""
...
def test_kimi_coding_strips_signed_thinking():
...
def test_kimi_model_name_on_foreign_gateway_preserves_only_unsigned_thinking():
    ...
    for model in ("kimi-k2.5", "k3"):
        ...
        assert _thinking_on_replay(DEEPSEEK, signature="", model=model), model


def test_direct_anthropic_keeps_signed_on_latest():
    assert _thinking_on_replay(None, model="claude-sonnet-4-20250514")
...
def test_orphan_tool_turn_demotes_and_leaks_no_internal_marker():
    ...
    is stripped because the third-party endpoint cannot validate it."""
    response = SimpleNamespace(
        content=[
    ...
    assert "thinking" not in types, types
