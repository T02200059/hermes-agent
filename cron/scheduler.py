...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    message = t("cron.failure_generic", job_name=job_name, details=cleaned)
...
# CR-002 / WR-03: owner/scripts/ basename allowlist with mtime refresh.
...
def _get_owner_scripts_allowlist() -> set:
    ...
    try:
        current_mtime = owner_scripts.stat().st_mtime if owner_scripts.is_dir() else None
    except OSError:
        current_mtime = None
    if _OWNER_SCRIPTS_ALLOWLIST is None or current_mtime != _OWNER_SCRIPTS_DIR_MTIME:
        basenames: set = set()
        if owner_scripts.is_dir():
            for file_path in owner_scripts.rglob("*"):
                if file_path.is_file() and file_path.suffix.lower() in {".py", ".sh", ".bash"}:
                    basenames.add(file_path.name)
...
def _run_job_script_with_claim_heartbeat(
    ...
    def _run() -> tuple[bool, str]:
        # Preserve the legacy one-argument call when no job args exist. A
        # number of integrations monkeypatch this boundary with that stable
        # signature, while jobs that declare args still use the new path.
        if args is None and workdir is None and cancel_event is None:
            return _run_job_script(script_path)
        return _run_job_script(
            script_path, args=args, workdir=workdir, cancel_event=cancel_event
        )
...
def _build_job_prompt(
    ...
    if script_path:
        if prerun_script is not None:
            ...
            success, script_output = _run_job_script_with_claim_heartbeat(
                job, script_path, args=job.get("args")
            )
        if success:
            if script_output:
                prompt = (
    ...
    *,
    ...
    if job.get("no_agent"):
        ...
        try:
            ok, output = _run_job_script_with_claim_heartbeat(
                job, script_path, args=job.get("args"), workdir=_job_workdir,
    ...
    if script_path:
        prerun_script = _run_job_script_with_claim_heartbeat(
            job, script_path, args=job.get("args"), cancel_event=cancel_event,
        )
...
"""
        ...
        # [owner] cron-env-leak: defensive env scrub (ContextVar already reset above via main path)
        try:
            from owner.cron.run_job_hook import owner_cron_session_exit
            owner_cron_session_exit(None)
        except Exception:
            pass
