...
class QQAdapter(BasePlatformAdapter):
    ...
    async def disconnect(self) -> None:
        ...
        # Yield the event loop — let _reconnect() check _stop_retry before
        # it hits the next await point (asyncio.sleep in the backoff loop).
    ...
    async def _rebuild_http_client(self) -> None:
        ...
        # Clear cached token so _ensure_token() forces a fresh OAuth2 exchange
        self._access_token = None
        self._token_expires_at = 0.0
    ...
    async def _reconnect(self, backoff_idx: int) -> bool:
        ...
        while slept < delay:
            if self._stop_retry:
                logger.info("[%s] _stop_retry set during backoff — aborting reconnect", self._log_tag)
                return False
        ...
        try:
            while self._running:
                ...
                try:
                    ...
                    hb_fail = 0
                    logger.info(
                        "[%s] Heartbeat sent (seq=%s, interval=%.1fs)",
                        self._log_tag, self._last_seq, self._heartbeat_interval,
                    )
                except Exception as exc:
                    hb_fail += 1
                    logger.warning(
