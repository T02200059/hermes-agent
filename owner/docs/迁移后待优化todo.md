...
- ✅ **FeishuUserStore Phase A/B/C**（2026-06-15）
  - Phase A：`user_store.py` facade，`_client` setter 同步 `bind_client()`
  - Phase B：`FeishuUserEntry.name` → `display_name`；resolve/seed 时同步到 user record
  - Phase C：磁盘 v2（`{_version:2, users:{...}}`）+ v1 自动迁移；名称解析内聚到 `FeishuUserStore`；移除 adapter `_name_cache` / `_sender_name_cache` shim；`sender_name_cache.py` 仅保留 legacy 测试 fallback
