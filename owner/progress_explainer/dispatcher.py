...
class ProgressExplainer:
    ...
    def __init__(
        ...
        self._platform = str(getattr(source, "platform", "") or "")
        self._chat_id = str(getattr(source, "chat_id", "") or "")
        self._session_id = str(getattr(agent, "session_id", "") or "") or str(
            getattr(turn_ctx, "session_id", "") or ""
        ) if (getattr(agent, "session_id", None) or getattr(turn_ctx, "session_id", None)) else str(
            getattr(source, "session_id", "") or ""
        )
        self._turn_id = str(getattr(agent, "_current_turn_id", None) or "")
    ...
    def install(self) -> "ProgressExplainer":
        ...
        # on_stream_delta 观察者（模块级表）
        _register_tracker(self._session_id, self._turn_id, tracker)

        tick_seconds = self._tick_seconds()
        self._tick_task = asyncio.create_task(self._tick_loop())
        logger.debug(
            "progress_explainer installed (platform=%s chat=%s tick=%ss)",
            self._platform,
            self._chat_id,
            tick_seconds,
        )
    ...
    def _activity_summary(self) -> Dict[str, Any]:
        try:
            a = self._agent.get_activity_summary()
