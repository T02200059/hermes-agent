...
def _hour_start(path: str) -> int:
    ...
    >>> _hour_start('hermes_20260503_14.csv')  # 返回值取决于本地时区
    1746280800  # UTC+8: 2026-05-03 14:00:00；其他时区不同
    """
...
def _aggregate(rows: list[dict]) -> dict:
    ...
    # [review] WR-02: 防御空列表 — max([])/min([]) 抛 ValueError
    if not rows:
        return {"avg_cpu": 0, "max_cpu": 0, "min_cpu": 0, "avg_mem": 0, "max_mem": 0, "min_mem": 0, "avg_rss": 0, "max_rss": 0, "min_rss": 0, "avg_vsz": 0, "sample_count": 0}
...
def process_hourly_files():
    ...
    # [review] WR-01: 读取已有 daily 数据，按 day_ts 去重，防止重复追加
    day_file = os.path.join(DAY_DIR, "daily.csv")
    existing_days: dict[int, list] = {}
    if os.path.isfile(day_file):
        with open(day_file, "r") as f:
            for r in csv.DictReader(f):
                day_ts_val = int(float(r["day_ts"]))
                existing_days[day_ts_val] = [r[k] for k in r.fieldnames]
        ...
        try:
            ...
            with open(day_file, "a", newline="") as f:
                ...
                for day_ts in sorted(groups):
                    # [review] WR-01: 跳过已有数据，防止重复追加
                    if day_ts in existing_days:
                        continue
                    grp = groups[day_ts]
                    w.writerow([
                        day_ts,
