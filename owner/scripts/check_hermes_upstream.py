...
def check_updates():
    ...
    # Get current branch HEAD (do not hard-code owner/custom branch names)
    current_branch, rc = run_git(["git", "branch", "--show-current"])
    if rc != 0 or not current_branch:
        current_branch = "HEAD"
    current_head, _ = run_git(["git", "rev-parse", current_branch])

    # Get upstream/main HEAD
    upstream_head, _ = run_git(["git", "rev-parse", "upstream/main"])

    # Get the merge base (common ancestor)
    merge_base, _ = run_git(["git", "merge-base", current_branch, "upstream/main"])

    if upstream_head == merge_base:
        # No new commits upstream
        return {
            "has_updates": False,
            "message": "✅ Hermes 官方仓库无新更新，当前分支已是最新。"
        }
...
💡 建议：
运行 `git fetch upstream` 后按当前迁移流程评估并合并 upstream/main。

⚠️ 注意：合并前请确保当前分支的修改已保存。"""
    }
