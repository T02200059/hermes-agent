...
class AIAgent:
    ...
    def _needs_kimi_tool_reasoning(self) -> bool:
        ...
        1. Main host/provider table (``matches_reasoning_echo_family("kimi", ...)``)
           — dedicated kimi-coding / official kimi+moonshot hosts.
        2. **[owner] Kimi-family model id on a non-aggregator host** (damodel /
           custom / ark, …). Aggregators (OpenRouter, Nous) re-export Kimi under
           *their* protocol and reject ``reasoning_content`` on input — skipped.
        ...
        # [owner] Layer 2: kimi-family model on company proxies (damodel, …)
        try:
        ...
        try:
            ...
            if (
                ...
                def _on_session_turn_lease_wait(elapsed: float) -> None:
                    ...
                    _lease_waited = True
                    if elapsed < 1.0:
                        self._emit_status(t("gateway.turn_lease.waiting"))
                    else:
                        self._emit_status(
                            t("gateway.turn_lease.still_waiting",
                              seconds=int(elapsed))
                        )
                ...
                if not _turn_db.acquire_session_turn_lease(
                    ...
                    if getattr(self, "_interrupt_requested", False):
                        ...
                        interrupt_msg = t("gateway.turn_lease.interrupted")
                        interrupt_result = {
                    ...
                    timeout_msg = t("gateway.turn_lease.timeout")
                    logger.error(
                ...
                if _lease_waited:
                    self._emit_status(t("gateway.turn_lease.free"))
