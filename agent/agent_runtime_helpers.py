...
def try_recover_primary_transport(
    ...
    try:
        ...
        agent.owner_provider_name = rt.get("owner_provider_name", agent.provider)
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    agent._primary_runtime = {
        "model": agent.model,
        "provider": agent.provider,
        # [owner] §2.1 owner_provider_name — getattr for __new__-constructed
        # agents (tests, partial inits); falls back to provider, matching
        # init_agent.py's `or agent.provider` default.
