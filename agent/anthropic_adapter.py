...
def _manage_thinking_signatures(
    ...
    for idx, m in enumerate(result):
        ...
        if _is_kimi_family_endpoint(base_url, model):
            ...
            pass
        elif _is_deepseek_anthropic_endpoint(base_url) or _is_minimax_anthropic_endpoint(base_url):
            # DeepSeek / MiniMax: strip signed, preserve unsigned.
            # [owner] MiniMax Anthropic endpoint needs unsigned thinking-block
            # round-trip (same contract as DeepSeek).
            new_content = []
            for b in m["content"]:
