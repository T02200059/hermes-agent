...
def _coerce_text_response(entry: _ClarifyEntry, response: str) -> Optional[str]:
    ...
    For multi-select clarifies (entry.multi_select=True):
      ...
      - Out-of-range numbers reject the whole reply (return None)
      - Selections are returned as a JSON array string
    ...
    if 0 <= idx < len(entry.choices):
        ...
        return get_choice_key(choice) or get_choice_display(choice)
...
def _coerce_multi_select_text(entry: _ClarifyEntry, text: str) -> Optional[str]:
    ...
    for token in tokens:
        ...
        for choice in choices:
            display = get_choice_display(choice)
            if token.casefold() == display.strip().casefold():
                matched = get_choice_key(choice) or display.strip()
                break
        if matched is None:
            return None
