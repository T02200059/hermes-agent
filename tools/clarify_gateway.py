...
class _ClarifyEntry:
    ...
    # tolerated — adapters should read get_choice_display(c) for rendering.
...
def _label_matches(text: str, choice: object) -> bool:
    ...
    label = get_choice_display(choice)
    return strip_recommended(text).casefold() == strip_recommended(label).casefold()
...
def _selection_attempt_tokens(
    ...
    if choices:
        max_choice_words = max(
            (len(get_choice_display(choice).split()) for choice in choices),
            default=1,
        )
...
def _coerce_text_response_detailed(
    ...
    Choice items may be plain strings (legacy) or normalized
    ``{"display", "key"}`` dicts from owner.clarify.choice_normalizer; dicts
    resolve to key (fallback display) so the model sees stable identifiers.
    ...
    if is_int and 0 <= idx < len(entry.choices):
        choice = entry.choices[idx]
        return get_choice_key(choice) or get_choice_display(choice), None

    for choice in entry.choices:
        if _label_matches(text, choice):
            return get_choice_key(choice) or get_choice_display(choice).strip(), None
...
def _coerce_multi_select_text(entry: _ClarifyEntry, text: str) -> Optional[str]:
    """Parse a typed multi-select reply into a JSON array of choice keys/labels.

    Accepts numbers and/or exact labels separated by commas (and, for
    all-numeric replies, bare spaces): "1,3", "1 3", "staging, prod".
    Returns ``None`` when any token is out of range or unrecognised.
    """
    ...
    for token in tokens:
        if token.isdigit():
            ...
            if 0 <= idx < len(choices):
                label = get_choice_key(choices[idx]) or get_choice_display(choices[idx])
        ...
        for choice in choices:
            if _label_matches(token, choice):
                matched = get_choice_key(choice) or get_choice_display(choice).strip()
                break
        if matched is None:
            return None
