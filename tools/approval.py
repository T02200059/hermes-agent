...
def _is_verification_artifact_cleanup(command: str) -> bool:
    ...
    # macOS exposes the conventional /tmp path as a system symlink to
    # /private/tmp. Accept that well-known spelling, but do not grant the
    # same exemption to an arbitrary symlink supplied as the temp directory.
    allowed_dirs = {temp_dir}
    if raw_temp_dir == "/tmp":
        allowed_dirs.add(raw_temp_dir)
    if operand not in {os.path.join(path, basename) for path in allowed_dirs}:
        return False
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if approval_mode == "smart":
        ...
        if verdict == "approve":
            ...
            return {
                "approved": False,
                "message": (
                    t("approval.smart_denied", description=combined_desc_for_llm)
                    + breaker_addendum
                ),
                "smart_denied": True,
            }
    ...
    if choice == "deny":
        ...
        return {
            "approved": False,
            "message": t("approval.cli_denied") + breaker_addendum,
            "pattern_key": primary_key,
            "description": combined_desc,
            "outcome": "denied",
            "user_consent": False,
    ...
    if approval_mode == "smart":
        ...
        if verdict == "deny" and not (is_gateway or is_ask):
            ...
            return {
                "approved": False,
                "message": (
                    t("approval.execute_code_smart_denied") + breaker_addendum
                ),
                "smart_denied": True,
                "pattern_key": pattern_key,
                "description": description,
