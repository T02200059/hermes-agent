...
const renderIndicator = (style: IndicatorStyle, tick: number, themeSpinner: ThemeSpinner): IndicatorRender => {
  if (style === 'kaomoji') {
    return { frame: themeSpinner.waitingFaces[tick % themeSpinner.waitingFaces.length] ?? '', intervalMs: FACE_TICK_MS, showVerb: true }
  }
...
function FaceTicker({ color, startedAt, spinner }: { color: string; startedAt?: null | number; spinner: ThemeSpinner }) {
  ...
  const [verbTick, setVerbTick] = useState(() => Math.floor(Math.random() * spinner.thinkingVerbs.length))
  ...
  // timer dangling.
  const { intervalMs, showVerb } = renderIndicator(style, 0, spinner)
  ...
  const { frame } = renderIndicator(style, tick, spinner)
  const verb = spinner.thinkingVerbs[verbTick % spinner.thinkingVerbs.length] ?? ''
