...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    return (
        ...
        "Local auto-start disabled - OpenViking server is managed externally via Docker.",
    )
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def _commit_session(
        ...
        *,
        ...
        # POV.  Matches the existing _finalize_session_async guard.
        if self._shutting_down:
            return False
        try:
            self._client.post(
            ...
            if clear_missing and not isinstance(e, KeyboardInterrupt) and _status_code_from_error(e) == 404:
                self._clear_pending_session(sid)
