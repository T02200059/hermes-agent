...
# [owner] _approval_label and approval card builders moved to owner/feishu/approval.py
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # [owner] guide card state: guide_id -> {source}
        self._guide_card_state: Dict[str, Dict[str, Any]] = {}
        # Feishu reaction deletion requires the opaque reaction_id returned
        # by create, so we cache it per message_id.
        self._pending_processing_reactions: "OrderedDict[str, str]" = OrderedDict()
        # [owner] early-typing: track background tasks to prevent GC before completion.
    ...
    async def send_exec_approval(
        ...
        try:
            # [owner] approval: pre-warm open_id -> 中文名 for zero-delay callback
            # (see owner/feishu/sender_name_helpers.py)
    ...
    async def _handle_message_with_guards(self, event: MessageEvent) -> None:
        ...
        # [owner] early-typing: fire typing reaction when chat_lock is held
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        metadata = dict(extra_metadata or {})  # [owner] carry sub-profile chat_type/open_id
        if thread_id:
