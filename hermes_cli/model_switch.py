...
def list_authenticated_providers(
    ...
    *,
    ...
    Discovery is performed in three layers with early exit:

    1. Layer 1 emits every provider explicitly listed in ``user_providers``
       using the configured model list. No credential probing is done for
       these rows — the user's explicit configuration is trusted.
    2. If Layer 1 emitted all configured providers and ``refresh`` is false,
       the function returns early without scanning built-in providers.
    3. Otherwise Layer 2 scans built-in/overlay/canonical providers for cheap
       env-var credential signals only.
    4. Layer 3 performs the slower auth-store / credential-pool checks for
       candidates that were not authenticated by Layer 2, running the checks
       in parallel.

    Unified slug/mdev_id deduplication is applied across all layers so e.g.
    ``kimi-coding`` / ``kimi-coding-cn`` or ``github-copilot`` / ``copilot``
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            if not models_list:
                ...
                if "api.openai.com" in url_lower:
                    # Defer to the canonical curated list; curated is built
                    # lazily below only when discovery layers run.
                    fb = _PROVIDER_MODELS.get("openai") or []
                    if fb:
                        models_list = list(fb)
            ...
            # Record the models.dev mapping for this slug so later layers do
            # not emit a duplicate built-in row for the same endpoint.
            if ep_name.lower() in PROVIDER_TO_MODELS_DEV:
                seen_mdev_ids.add(PROVIDER_TO_MODELS_DEV[ep_name.lower()])
            elif ep_name.lower() in PROVIDER_TO_MODELS_DEV.values():
                seen_mdev_ids.add(ep_name.lower())
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        if _configured_count > 0 and len(results) >= _configured_count and not refresh:
            _skip_discovery = True

    if not _skip_discovery:
        # -------------------------------------------------------------------
        # Data preparation: only needed when discovery layers run.
        # -------------------------------------------------------------------
            ...
            *,
            ...
            if overlay and overlay.auth_type == "aws_sdk":
                ...
                return ids if ids else (
                    curated.get(slug, [])
                    or (curated.get(mdev_id, []) if mdev_id else [])
                )
        ...
        def _has_env_creds(
            ...
            *,
            ...
            for env_vars in env_vars_list:
                if any(os.environ.get(ev) for ev in env_vars):
                    return True
            return False
        ...
        def _has_auth_creds(
            ...
            *,
            ...
            if providers_store.get(slug) or (mdev_id and providers_store.get(mdev_id)):
                return True
        ...
        # -------------------------------------------------------------------
        # Pre-compute credential-signal set (Layer 2/3 short-circuit)
        # -------------------------------------------------------------------
        ...
        # Anthropic has external credential files read outside the auth store.
        ...
        # -------------------------------------------------------------------
        Layer3Candidate = NamedTuple("Layer3Candidate", [
            ("slug", str),
            ("mdev_id", Optional[str]),
            ("overlay", Optional[object]),
            ("cp_config", Optional[object]),
