"""新对话时展示的随机提示，帮助用户发现功能。"""

import random


# ---------------------------------------------------------------------------
# Tip corpus — 涵盖 slash 命令、CLI 参数、配置、快捷键、工具、gateway、
# skills、profiles 和工作流技巧的单行提示。
# ---------------------------------------------------------------------------
...
TIPS = [
    ...
    "裸 sudo 命令会自动改写为从 .env 管道传入 SUDO_PASSWORD — 无需交互式提示。",
    ...
    "agent.tool_use_enforcement 引导描述动作而不调用工具的模型 — GPT/Codex 自动启用。",
    ...
    "每个 profile 拥有独立的子进程 HOME 在 HERMES_HOME/home/ — 隔离的 git、ssh、npm、gh 配置。",
...
def get_random_tip(exclude_recent: int = 0) -> str:
    ...
    Args:
        exclude_recent: 当前未使用；保留供未来跨会话去重。
    """
